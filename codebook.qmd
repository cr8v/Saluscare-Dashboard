...
```{r}
df_codebook[[8]] %>%
  mutate(TODD_Group = case_when(grepl("9999",TODD)~"Current", TRUE ~ "Before")) %>%
  mutate(
...
f_reactable(
  ...
  columns = list(TODD_Group = colDef(align="left"),
  ...
  groupBy = c("TODD_Group", "SICKCD_Group", "SICKCD_Subgroup", "SICKCD_Subgroup2"),
  pagination = FALSE,
  compact = TRUE,
  wrap = FALSE,
  width = "100%"
...
df_codebook[[4]] %>% 
  mutate(TODD_Group = case_when(grepl("9999",TODD)~"Current", TRUE ~ "Before")) %>%
  mutate(PRCPCD_Group = factor(case_when(str_detect(PRCPCD, "^\\d") ~ "0-9",
  ...
  arrange(PRCPCD_Group, PRCPCD) %>%
  f_reactable(
    columns = list(TODD_Group = colDef(align="left"),
                   PRCPCD_Group  = colDef(name = "1st", align = "left", maxWidth = 100),
    ...
    groupBy = c("TODD_Group","PRCPCD_Group", "PRCPCD_Group2", "PRCPCD_Group3"),
    pagination = FALSE,
    compact = TRUE,
    wrap = FALSE
    )
...
df_codebook[[5]] %>% 
  mutate(TODD_Group = case_when(grepl("9999",TODD)~"Current", TRUE ~ "Before")) %>%
  mutate(INQRFLAG_Group = str_extract(INQRFLAG, "^[A-Z]+")) %>%
  select(TODD_Group, INQRFLAG_Group, INQRFLAG, CDNAME, FROMDD, TODD, everything()) %>%
  arrange(INQRFLAG) %>%
  f_reactable(columns = list(TODD_Group = colDef(align="left"),
                             INQRFLAG_Group = colDef(align = "left"),
                             INQRFLAG = colDef(style = f_style_sticky)
                             ),
              groupBy = c("TODD_Group","INQRFLAG_Group"),
              pagination = FALSE,
              height = 500
            )
```
...
```{r}
df_codebook[[6]] %>% select(APPYEAR, HEALEXAMFLAG, INQRFLAG, SUPINQRCD, INQRCD, INQRNM) %>% 
  mutate(SUPINQRCD_Group = case_when(SUPINQRCD == "*" ~ "*", TRUE ~ "_*"),
         SUPINQRCD_Group2 = nchar(INQRCD)) %>%
  arrange(desc(APPYEAR),HEALEXAMFLAG,INQRFLAG) %>%
  #filter(HEALEXAMFLAG == "C", SUPINQRCD == "*") %>%
  f_reactable(
    columns = list(
      APPYEAR = colDef(defaultSortOrder = "desc"),
      INQRFLAG = colDef(align="left"),
      SUPINQRCD = colDef(align="left"),
      INQRNM = colDef(aggregate = "unique")),
    groupBy = c("APPYEAR","HEALEXAMFLAG","INQRFLAG","SUPINQRCD_Group","SUPINQRCD_Group2","SUPINQRCD"),
    defaultSorted = c("APPYEAR","SUPINQRCD"),
    wrap = FALSE,
    height = 500
  )
```
...
```{r}
df_codebook[[6]] %>% 
  arrange(desc(APPYEAR), INQRCD, HEALEXAMFLAG, INQRFLAG) %>%
  f_reactable(
    columns = list(HEALEXAMFLAG = colDef(align = "left"),
                   INQRFLAG = colDef(align = "left"),
                   SUPINQRCD = colDef(align = "left"),
                   APPYEAR = colDef(align = "left", defaultSortOrder = "desc"),
                   INQRCD = colDef(align = "left", defaultSortOrder = "asc"),
                   INQRNM = colDef(align = "left")
                   ),
    groupBy = c("APPYEAR","USEYN","HEALEXAMFLAG","INQRFLAG","SUPINQRCD"),
    defaultSorted = c("APPYEAR","INQRCD"),
    wrap = FALSE,
    pagination = FALSE
    )
```
...
df_codebook[[10]] %>% 
  mutate(TODD_Group = case_when(grepl("9999",TODD)~"Current", TRUE ~ "Before")) %>%
  mutate(JUDGFLAG_Group = str_extract(JUDGFLAG, "^[A-Z|a-z]")) %>%
  arrange(JUDGFLAG) %>%
  f_reactable(
    groupBy = c("TODD_Group","JUDGFLAG_Group"),
    pagination = FALSE,
    height = 500
  )
```
...
df_codebook[[11]] %>% 
  mutate(TODD_Group = case_when(grepl("9999",TODD)~"Current", TRUE ~ "Before")) %>%
  mutate(JUDGCD_Group = str_extract(JUDGCD,"^[A-Z|0-9]"),
         JUDGCD_Group2 = str_extract(JUDGCD,"^[A-Z]+")
         ) %>%
  arrange(JUDGCD) %>%
  f_reactable(
    columns = list(JUDGNM = colDef(aggregate = "unique")),
    groupBy = c("TODD_Group","HEALEXAMFLAG","HEALEXAMDETLFLAG",
                "JUDGCD_Group","JUDGCD_Group2"),
    wrap = FALSE,
    height = 500
  )
```
